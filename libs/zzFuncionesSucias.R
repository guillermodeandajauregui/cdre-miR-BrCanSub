...
#libraries
#conchinero porque ese cluster del INMEGEN es un muladar
...
discretizer <- function(expmatrix){
  ...
  my_discrete <- lapply(X = my_index, FUN = function(i){
    ...
    my_row <- infotheo::discretize(X = my_row, disc = "equalfreq")
    
  })
  return(my_discrete)
}

par_discretizer <- function(expmatrix, korez = 2){
  my_index <- 1:nrow(expmatrix)
  names(my_index) <- dplyr::pull(expmatrix, 1)
