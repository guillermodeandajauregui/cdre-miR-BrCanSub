...
#get the groups for all enriched functions

List_reducedEnrichmentGroups <- lapply(X = EnrichmentList, FUN = function(subtype){
  lapply(X = subtype, FUN = function(cdre.rich){
    reducedEnrichment_groups(rich.df = cdre.rich, 
                      GO.ID.row   = "rowname",
                      sim.measr   = "Wang", 
                      SemData     =  hsGO, 
                      dist.mthd   = "euclidean",
...
saveRDS(object = List_reducedEnrichmentGroups, file = "List_reducedEnrichmentGroups.RDS")
